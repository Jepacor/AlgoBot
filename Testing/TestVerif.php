...
class TestVerif
{
    ...
    public static function getTestsNiveau($niveau) : void
    {
        ...
        $resultrequete = MyPDO::requeteStandard("SELECT Id_Resultats FROM Resultats WHERE Niveau = :niveau",
            array(":niveau" => $niveau));
        foreach ($resultrequete as $ligne){
            $resultsortie = MyPDO::requeteStandard("SELECT Sortie FROM Sorties WHERE Id_Resultats = :id_result",
                array(":id_result" => $ligne['Id_Resultats']));
            foreach ($resultsortie as $sortie){
                $sorties->append($sortie["Sortie"]);
            }
            $resultentree = MyPDO::requeteStandard("SELECT Entree FROM Entrees WHERE Id_Resultats = :id_result",
                array(":id_result" => $ligne['Id_Resultats']));
    ...
    {
        $sortie = MyPDO::requeteStandard("SELECT Sortie FROM Sorties WHERE Id_Resultats = :id_result",
    ...
    {
        $resultats_niveau = MyPDO::requeteStandard("SELECT Id_Resultats FROM Resultats WHERE Niveau = :id_niveau",
